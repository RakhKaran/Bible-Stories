...
export default function StoryQuestionsQuickEditForm({ currentQuestionId, open, onClose, onRefreshStoriesQuestions,  languages }) {
  ...
  const fetchStoryQuestionById = async () => {
    try{
      const response = await axiosInstance.get(`/story-question-by-id-admin/${currentQuestionId}`);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const inputData = {
        ...
        audios : data.audios,
        storiesId : currentQuestion.storiesId
      };

      const response = await axiosInstance.patch(`/stories-questions/${currentQuestionId}`, inputData);
      if(response?.data?.success){
        reset();
