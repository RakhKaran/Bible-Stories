...
export default function StoryQuestionsNewEditForm({ currentQuestion, storyId }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const inputData = {
        ...
        audios : data.audios,
        storiesId : Number(storyId),
      };

      const response = await axiosInstance.post(`/stories-questions`, inputData);
      if (response?.data?.success) {
        reset();
        enqueueSnackbar('Question added!');
        router.replace(paths.dashboard.story.questionList(storyId));
      }
    } catch (error) {
      console.error(error);
...
StoryQuestionsNewEditForm.propTypes = {
