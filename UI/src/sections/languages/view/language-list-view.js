...
const TABLE_HEAD = [
  { id: 'langName', label: 'Language' },
  { id: 'nativeLangName', label: 'Native Language'},
  { id: 'code', label: 'Code'},
...
export default function LanguageListView() {
  ...
  const {languages, languagesEmpty, refreshLanguages} = useGetLanguageList();

  useEffect(() => {
    if(languages && !languagesEmpty){
      setTableData(languages);
    }
                  ...
                  {dataFiltered
                    ...
                    .map((row) => (
                      <LanguageTableRow
                        ...
                        onRefreshLanguages={() => refreshLanguages()}
                      />
                    ))}
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (lang) => lang.langName.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }
